...
(defn open_replica
      "Opens a connection to a node in the replica set mode."
      [test node port]
      ;; (let [nodelist (:nodes test)
      ;;                                         nodelist1 (str/join (str ":" port ",") nodelist)
      ;;                                         ;; nodelist1 (str/join (str ":" port ",") nodelist)
      ;;                                         nodelist2 (str "mongodb://" nodelist1)
      ;;                                         nodelist3 (str nodelist2 ":" port)
      ;;                                         nodelist4 (str nodelist3 "/?readPreference=nearest")]
      ;; (def connectionString nodelist4)
      (MongoClients/create
        (.. (MongoClientSettings/builder)
            (applyToClusterSettings (with-block builder
                                                (.. builder
           ...
           (loop []
             (or (try
                   (let [conn (open node port)
                         db (.getDatabase conn "test")]
                     (try
                       ;(.first (.listDatabaseNames conn))
                       ;; For version 4.4.5, the (.first (.listDatabaseNames conn)) command will block here, so we replace it with the "buildInfo" one.
                       ;; It works but I do not known why it works. Our (info) will not display in jepsen.log, which is a question.
                       (let [build-info (.runCommand db (new Document "buildInfo" 1))]
                            (info "here!" (.toJson build-info)))
                       conn
