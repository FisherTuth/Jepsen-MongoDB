...
(def deployments
  {:sharded-cluster db/sharded-db
   :replica-set     repl/replica-db})
(def all-workloads
...
(defn mongodb-test
  ...
  (let [workload-name (:workload opts)
        ...
        db            (if (db/replica-set-deployment? opts)
                        (repl/replica-db opts)
                        (db/sharded-db opts))
    ...
    (println "===================asdsadas""this is db in mongodb-test" db)
    (merge tests/noop-test
           opts
           {:name (str "mongodb " (name workload-name)
                       (if (db/replica-set-deployment? opts)
                         (str " replica-set")
                         (str " sharded-cluster"))
...
(def cli-opts
  ...
  [[nil "--nemesis FAULTS" "A comma-separated list of nemesis faults to enable"
     ...
     :validate [(partial every? #{:pause :kill :partition :clock :member :island})
   ...
   ["-d" "--deployment DEPLOYMENT" "Which deployment should we configure? replica or shard?"
    :parse-fn keyword
    :validate [deployments (cli/one-of deployments)]]
   [nil "--m-type NUM" "member-type"
    :default 0
    :parse-fn parse-long
